#!/usr/bin/env bash
# Discover existing AWS resources via AWS CLI and emit inventory JSON.
# Output: JSON document with resources[].{type,name,id,service}
set -euo pipefail
...
# comma-separated services; default = common set
...
Discover AWS resources and print inventory JSON (or write to -o FILE).

Options:
  -r, --region REGION     AWS region (default: ${AWS_REGION})
  -s, --services LIST     Comma-separated services to scan (default: common set)
                          Available: vpc,subnet,route_table,igw,nat,eip,sg,ec2,ebs,
                                     s3,rds,dynamodb,lambda,elb,iam_role
                          Use "all" for the full default set + iam_role
  --skip-bucket NAME      Exclude this S3 bucket (e.g. state bucket)
  --skip-table NAME       Exclude this DynamoDB table (e.g. lock table)
  -o, --output FILE       Write JSON to file instead of stdout
  -n, --dry-run           Emit sample inventory without calling AWS
  -h, --help              Show help
EOF
}
...
# Accumulate resources as JSON lines: {"type":"...","name":"...","id":"...","service":"..."}
...
add_resource() {
  ...
  local name
  # Prefer human-readable prefix from type + sanitized id
  name="$(tf_name "${type#aws_}_${id}")"
  # Avoid "vpc_vpc_..." style duplication when id already contains the type prefix
  case "${id}" in
...
safe_aws() {
  # Run aws; on failure warn and return empty
  if ! "$@" 2>/tmp/aws-discover-err.txt; then
    warn "AWS call failed: $* — $(tr '\n' ' ' </tmp/aws-discover-err.txt)"
    return 1
  fi
}
...
discover_live() {
  ...
  export DISCOVERED_ACCOUNT_ID="${account_id}"

  if want vpc; then
    log "Scanning VPCs..."
    local ids
    ids="$(safe_aws aws ec2 describe-vpcs --region "${AWS_REGION}" --query 'Vpcs[].VpcId' --output text || true)"
    for id in ${ids:-}; do
  ...
  if want subnet; then
    ...
    local ids
    ids="$(safe_aws aws ec2 describe-subnets --region "${AWS_REGION}" --query 'Subnets[].SubnetId' --output text || true)"
    for id in ${ids:-}; do
  ...
  if want route_table; then
    ...
    local ids
    ids="$(safe_aws aws ec2 describe-route-tables --region "${AWS_REGION}" --query 'RouteTables[].RouteTableId' --output text || true)"
    for id in ${ids:-}; do
  ...
  if want igw; then
    ...
    local ids
    ids="$(safe_aws aws ec2 describe-internet-gateways --region "${AWS_REGION}" --query 'InternetGateways[].InternetGatewayId' --output text || true)"
    for id in ${ids:-}; do
  ...
  if want nat; then
    ...
    local ids
    ids="$(safe_aws aws ec2 describe-nat-gateways --region "${AWS_REGION}" \
  ...
  if want eip; then
    ...
    local ids
    ids="$(safe_aws aws ec2 describe-addresses --region "${AWS_REGION}" --query 'Addresses[].AllocationId' --output text || true)"
    for id in ${ids:-}; do
  ...
  if want sg; then
    ...
    local ids
    ids="$(safe_aws aws ec2 describe-security-groups --region "${AWS_REGION}" --query 'SecurityGroups[].GroupId' --output text || true)"
    for id in ${ids:-}; do
  ...
  if want ec2; then
    ...
    local ids
    ids="$(safe_aws aws ec2 describe-instances --region "${AWS_REGION}" \
  ...
  if want ebs; then
    ...
    local ids
    ids="$(safe_aws aws ec2 describe-volumes --region "${AWS_REGION}" --query 'Volumes[].VolumeId' --output text || true)"
    for id in ${ids:-}; do
  ...
  if want s3; then
    ...
    local names
    names="$(safe_aws aws s3api list-buckets --query 'Buckets[].Name' --output text || true)"
    for name in ${names:-}; do
      [[ -z "${name}" || "${name}" == "None" ]] && continue
      [[ -n "${SKIP_BUCKET}" && "${name}" == "${SKIP_BUCKET}" ]] && continue
      # Prefer buckets in target region when LocationConstraint is readable
      local loc
      loc="$(aws s3api get-bucket-location --bucket "${name}" --query 'LocationConstraint' --output text 2>/dev/null || echo "unknown")"
  ...
  if want elb; then
    ...
    local arns
    arns="$(safe_aws aws elbv2 describe-load-balancers --region "${AWS_REGION}" \
  ...
  if want iam_role; then
    log "Scanning IAM roles (excluding AWS service-linked)..."
    local names
    names="$(safe_aws aws iam list-roles --query 'Roles[?!starts_with(RoleName, `AWSServiceRole`)].RoleName' --output text || true)"
    local count=0
    for name in ${names:-}; do
      [[ -z "${name}" || "${name}" == "None" ]] && continue
      add_resource "aws_iam_role" "${name}" "iam_role"
      count=$((count + 1))
      # Cap to avoid huge accounts exploding imports
      if [[ "${count}" -ge 100 ]]; then
        warn "IAM roles capped at 100 — pass a narrower scan later if needed"
        break
      fi
    done
  fi
...
if [[ "${DRY_RUN}" == true ]]; then
  ...
  aws sts get-caller-identity --output table >&2 || die "AWS credentials are invalid or missing"
  discover_live
fi

# Deduplicate names (collision -> append short hash of id)
DEDUPED="$(mktemp)"
...
log "Discovered ${COUNT} resources in ${AWS_REGION}" >&2
